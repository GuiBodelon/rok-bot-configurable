...
class StateStore:
    """
    Fonte única de verdade do runtime.

    Responsabilidades:
    - manter o estado atual da execução
    - permitir leitura segura por snapshot
    - permitir updates parciais e atômicos
    - resetar estado operacional sem perder flexibilidade
    """
        ...
        with self._lock:
            for key, value in kwargs.items():
                if not hasattr(self._state, key):
                    raise AttributeError(f"Campo de RuntimeState inexistente: '{key}'")
